...
gpuConfig.allow_soft_placement = True                           #设置为True，当GPU不存在或者程序中出现GPU不能运行的代码时，自动切换到CPU运行gpuConfig.gpu_options.allow_growth = True                       #设置为True，程序运行时，会根据程序所需GPU显存情况，分配最小的资源
...
class Train():
    ...
    def train(self):
        ...
        # 构建训练数据集
        train_data_sentences, train_data_labels, train_seq_length = data_transform(train_sentences, train_labels, word_to_id, label_to_id)
        test_data_sentences, test_data_labels, test_seq_length = data_transform(test_sentences, test_labels, word_to_id, label_to_id)

        # 打印训练、测试数据量，数据与标签量是否相等
        logging.info('Train Data: {}'.format(np.array(train_data_sentences).shape))
        logging.info('Train Label: {}'.format(np.array(train_data_labels).shape))
        logging.info('Test Data: {}'.format(np.array(test_data_sentences).shape))
        logging.info('Test Label: {}'.format(np.array(test_data_labels).shape))

        # 数据集校验，数据和标签数量是否有误
        if (len(train_data_sentences) != len(train_data_labels)) or (len(test_data_sentences) != len(test_data_labels)):
        ...
        for epoch in range(1, config.epochs + 1):
            ...
            batch_train_data, batch_train_label, batch_train_seq_length = creat_batch_data(train_data_sentences,
                                                                                           train_data_labels,
                                                                                           train_seq_length,
                                                                                           config.batch_size)
            for step, (batch_x, batch_y, batch_length) in tqdm(enumerate(zip(batch_train_data, batch_train_label, batch_train_seq_length))):
                feed_dict = {self.model.input_x: batch_x,
                             self.model.input_y: batch_y,
                             self.model.input_length: batch_length,
                ...
                # 计算准确率
                for predict, input_x, input_y, seq_length in zip(train_predict, batch_x, batch_y, batch_length):
                    ...
                    if len(train_intersection)!=0:
                        train_precision = len(train_intersection)/len(train_entity_predict)             # Precision
                        train_recall = len(train_intersection)/len(train_entity_y)                      # Recall
                        train_f1 = (2*train_precision*train_recall)/(train_precision+train_recall)      # F1
            ...
            batch_test_data, batch_test_label, batch_test_seq_length = creat_batch_data(test_data_sentences,
                                                                                        test_data_labels,
                                                                                        test_seq_length,
                                                                                        config.batch_size)
            for (batch_x, batch_y, batch_length) in tqdm(zip(batch_test_data, batch_test_label, batch_test_seq_length)):
                feed_dict = {self.model.input_x: batch_x,
                             self.model.input_y: batch_y,
                             self.model.input_length: batch_length,
                ...
                # 计算准确率
                for predict, input_x, input_y, seq_length in zip(test_predict, batch_x, batch_y, batch_length):
                    ...
                    test_intersection = [item for item in test_entity_y if item in test_entity_predict]
...
def data_transform(input_data, input_label, word_to_id, label_to_id):
    ...
    for sentence in tqdm(input_data):
        ...
        if sentence_length > config.seq_length:
            ...
            sequence_length.append(sentence_length)
    ...
    return datas, labels, seq_length


def creat_batch_data(input_data, input_label, input_length, batch_size):
    '''
    ...
    seq_length_shuffle = np.array(input_length)[indices]

    batch_data, batch_label, batch_seq_length = [], [], []
    ...
    return batch_data, batch_label, batch_seq_length
